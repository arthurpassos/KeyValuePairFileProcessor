...
namespace extractKV
{
    ...
    Configuration ConfigurationFactory::createWithEscaping(char key_value_delimiter, char quoting_character, const std::vector<char> & pair_delimiters)
    {
        ...
        {
            throw std::runtime_error ("Invalid arguments, {} is reserved for the escaping character");
//            throw Exception(
...
//                    ESCAPE_CHARACTER);
    ...
    void ConfigurationFactory::validate(char key_value_delimiter, char quoting_character, std::vector<char> pair_delimiters)
    {
        ...
        {
            throw std::runtime_error ("Invalid arguments, key_value_delimiter and quoting_character can not be the same");
        }
        ...
        if (pair_delimiters.size() > MAX_NUMBER_OF_PAIR_DELIMITERS)
        {
            ...
            throw std::runtime_error ("Invalid arguments, pair delimiters can contain at most {} characters");
        }

        if (pair_delimiters.empty())
        {
            throw std::runtime_error ("Invalid arguments, pair delimiters list is empty");
        }
        ...
        {
            throw std::runtime_error ("Invalid arguments, key_value_delimiter conflicts with pair delimiters");
        }
        ...
        {
            throw std::runtime_error ("Invalid arguments, quoting_character conflicts with pair delimiters");
        }
    }
